...
"""
update_openmeteo.py
Fetch hourly weather data from Open-Meteo with necessary parameters:
temperature, humidity, wind direction/speed, precipitation, light intensity,
UV radiation (NASA POWER fallback), pressure, soil moisture.

Saves CSV to:
    /Users/chandadiwakar/Downloads/Smart Produce/data/open_meteo.csv

Usage:
    python update_openmeteo.py [start_date] [end_date]
If dates omitted defaults to 2024-01-01 to yesterday.
"""
...
def fetch_nasa_uv_daily(start_date, end_date):
    # NASA POWER daily UV index fallback for missing uv_index from Open-Meteo
    params = {
    ...
    try:
        ...
        # convert YYYYMMDD keys to dates
        rows = []
...
def add_uv_to_df(df, start_date, end_date):
    # Add uv_index column by NASA POWER daily UV repeated hourly
